...
def register_user():
    ...
    register_button = tk.Button(register_window, text="Register", command=lambda: create_user(vorname_entry.get(), nachname_entry.get(), password_entry.get()))
    register_button.grid(row=4, column=1)

def create_user(vorname, nachname, password):
    ...
    if ( len(vorname) != 0) & (len(nachname) != 0):
        if re.search("^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{10,}$", password):
            ...
            sql = "INSERT INTO Mitarbeiter (Vorname, Nachname, Passwort) VALUES (%s, %s, %s)"
            val = (vorname, nachname, password)
